...
def filter_proteins_with_dot(
    input_fasta,
    output_fasta,
):
    """
    Remove protein sequences containing '.' and return removed IDs.

    Parameters
    ----------
    input_fasta : str
        Protein FASTA file generated by gffread (-y)
    output_fasta : str
        Output FASTA file without proteins containing '.'

    Returns
    -------
    removed_ids : list
        List of protein IDs that were removed
    """
...
def go_annotation(gff_refseq, ref_assembly, proteins_file_raw, proteins_file_rename, proteins_file, annotation_file,mrna_dict):

    tmp_path = os.path.dirname(proteins_file_raw)
    ...
    rename_fasta_headers(proteins_file_raw, proteins_file_rename, mrna_dict)

    removed_ids = filter_proteins_with_dot(proteins_file_rename, proteins_file)
...
def prepare_go_analysis(reference_genome, ID_ref_label, type_annotation_ref, ref_assembly,
    ...
    proteins_tmp_path = f"{go_path}/tmp"

    proteins_file_raw = f"{go_path}/tmp/{reference_genome}_raw.fasta"
    proteins_file_rename = f"{go_path}/tmp/{reference_genome}_renamed.fasta"
    ...
    go_annotation(gff_refseq, ref_assembly, proteins_file_raw, proteins_file_rename, proteins_file, annotation_file, mrna_dict)
