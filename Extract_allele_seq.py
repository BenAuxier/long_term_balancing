...
def find_read_corridinate(read, position):
    ...
    for info in read_cigar_list:
        ...
        if type_seq == "4":  # hard mask
            hard_mask = length_seq
        elif type_seq == "5":  # soft mask
            continue
        ...
        elif type_seq == 0:  # other situations, there are also type > 5, not considered
            length_reference += length_seq
...
def find_candidate_involvement(up_down_alignment,assembly_num):
    # get the random assembly list to test status
    selected_assemblies = random_select_assembly(up_down_alignment, assembly_num)
    all_status = {}
...
def analyze_all_candidate_position(candidate_data,annotation_sorted,bam_path,assembly_path,assembly_num, up_num, down_num, lower_limit):
    """
    ...
    for transcript_id,info in candidate_data.items():
        ...
        all_status = find_candidate_involvement(up_down_alignment,assembly_num)
...
def extract_allele_sequence(assembly_dir, candidate_gene, genome_accession, contig, start, end, type_seq, output_path):
    """
...
def find_allele_sequence_inbetween(assembly_dir,candidate_data_summary,output_path):
    seq_num = []
    interval_num = []
    for gene,summary in candidate_data_summary.items():
        for genome, info in summary["up_down_loci"].items():
            ...
            if seq == False:
                ...
                continue
            seq_num.append(gene)
            if info["interval difference(%)"] > 300:
                continue

            interval_num.append(gene)
            ...
            start_ref = min(upstream_read_position["start"], downstream_read_position["start"])
            end_ref = max(upstream_read_position["end"], downstream_read_position["end"])
            ...
            print(gene, genome)
            #print(upstream_rank, upstream_read_position)
            #print(downstream_rank, downstream_read_position)
            #for key, value in info.items():
                #print(key, value)
    print("seq_num",len(set(seq_num)))


def find_allele_sequence_inbetween(reference_genome, assembly_dir,candidate_data_summary,output_path):
    seq_num = []
    interval_num = []
    for gene,summary in candidate_data_summary.items():
        ...
        #print("ref info", start_ref, end_ref)
        extract_allele_sequence(
            assembly_dir,
            gene,
            reference_genome,
        ...
        #print(gene, summary["position_info"])
        for genome, info in summary["up_down_loci"].items():
...
#test the main code
candidate_data_test = dict(list(candidate_data.items())[0:10])
#print(candidate_data_test)
candidate_data_summary = analyze_all_candidate_position(candidate_data_test,
            annotation_sorted,bam_path,assembly_path,assembly_num, up_num, down_num, lower_limit)

gene_between = find_allele_sequence_inbetween(reference_genome, assembly_dir,candidate_data_summary,output_path)


#print("after depth",len(candidate_data_summary.keys()))
